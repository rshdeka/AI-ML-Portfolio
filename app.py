...
def experience_page():  
    ...
    - **Tech Stack**: Python, SQL, XGBoost, Azure Cosmos DB, Azure Databricks.
    - **Description**: Automated the data flow pipeline for Optum Bank's Customer Support data. The automated system consisted of loading Customer Support chats from Azure Databricks, 
    extraction of relevant data using Python and SQL, tagging chats into pre-defined categories by building a XGBoost multi-class classification model with GridSearchCV tuning, and 
    storage to Cosmos DB for further analysis. 
    Deployed into an Azure Function pipeline, and scheduled a Databricks job to run at regular intervals, to ensure that Cosmos DB is consistently updated with latest data. 
